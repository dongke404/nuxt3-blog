<script setup>
import { VALUABLE_LINKS } from '@/config'
const currIndex = ref(0)
const errMusic = ref(0)
const songList = ref([])
const currentSong = computed(() => {
  if (songList.value.length === 0) {
    return {
      name: '暂无歌曲',
      artist: '暂无歌曲',
      url: '',
      cover: '',
    }
  }
  else {
    return {
      name: songList.value[currIndex.value].name,
      artist: songList.value[currIndex.value].ar[0].name,
      url: `https://music.163.com/song/media/outer/url?id=${songList.value[currIndex.value].id}.mp3`,
      cover: songList.value[currIndex.value].al.picUrl,
    }
  }
})
const audio = ref(null)
const mLength = computed(() => songList.value.length)
const { data } = await useFetch(VALUABLE_LINKS.MUSIC163Detail)
if (data.value && data.value.code === 200) {
  const ids = data.value.playlist.trackIds
  const idslist = []
  for (const item of ids)
    idslist.push(item.id)
  const idstr = idslist.join(',')
  const { data: songDetails } = await useFetch(`https://music.kedong.me/song/detail?ids=${idstr}`)
  if (songDetails.value && songDetails.value.code === 200) {
    songList.value = songDetails.value.songs.sort(() => {
      return Math.random() > 0.5 ? -1 : 1
    })
  }
}

const ismuted = ref(true)
const ispaused = ref(true)

const playOrpluse = () => {
  if (audio.value.paused) {
    audio.value.play()
    ispaused.value = false
  }
  else {
    audio.value.pause()
    ispaused.value = true
  }
}
const playPre = () => {
  if (currIndex.value === 0)
    currIndex.value = mLength.value - 1

  else
    currIndex.value -= 1
  ispaused.value = false
  setTimeout(() => {
    audio.value.play()
  }, 1000)
}
const playNext = () => {
  if (currIndex.value === mLength.value - 1)
    currIndex.value = 0
  else
    currIndex.value += 1
  ispaused.value = false
  setTimeout(() => {
    audio.value.play()
  }, 1000)
}
const muted = () => {
  audio.value.muted = !audio.value.muted
  ismuted.value = !ismuted.value
}
const error = (e) => {
  if (errMusic.value === 10) {
    ispaused.value = true
    errMusic.value = 0
  }
  else {
    playNext()
    errMusic.value += 1
  }
}
</script>

<template>
  <client-only>
    <div id="player" class=" bg-main" :class="{ playing: !ispaused }">
      <div class="header-player">
        <audio
          ref="audio"
          :key="currentSong.url"
          :src="currentSong.url"
          name="media"
          @ended="playNext"
          @error="error"
        />
      </div>
      <div class="panel">
        <div class="control">
          <button
            class="prev-song button"
            @click.stop="playPre"
          >
            <Icon name="gg:play-track-prev" />
          </button>
          <button
            class="next-song button"
            @click.stop="playNext"
          >
            <Icon name="gg:play-track-prev" rotate="180deg" />
          </button>
          <button
            class="muted-toggle button"
            @click.stop="muted"
          >
            <Icon v-if="ismuted" name="ic:baseline-volume-mute" />
            <Icon v-else name="mdi:volume-mute" />
          </button>
          <button
            class="player button"
          >
            <nuxt-link :to="VALUABLE_LINKS.MUSIC163" target="_black">
              <svg t="1679477887825" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3176" width="32" height="32"><path d="M350.37500029 62h323.24999942C774.50000029 62 811.99999971 72.50000029 846.87499971 91.99999971a204.37499971 204.37499971 0 0 1 85.12500058 85.12500058c19.50000029 34.875 29.99999971 72.37500029 29.99999971 173.25v323.24999942c0 100.125-10.50000029 136.50000029-29.99999971 173.25a204.37499971 204.37499971 0 0 1-85.12500058 85.12500058c-37.50000029 19.50000029-73.125 29.99999971-173.25 29.99999971H350.37500029C249.49999971 962 212.00000029 951.49999971 177.12500029 932.00000029A204.37499971 204.37499971 0 0 1 91.99999971 846.87499971C72.50000029 811.99999971 62 774.50000029 62 673.62499971V350.37500029C62 249.49999971 72.50000029 212.00000029 91.99999971 177.12500029A204.37499971 204.37499971 0 0 1 177.12500029 91.99999971C212.00000029 72.50000029 249.49999971 62 350.37500029 62z" fill="#ce3633" p-id="3177" /><path d="M645.12500029 177.875a156.75000029 156.75000029 0 0 1 58.87499942 28.50000029 43.875 43.875 0 0 1 15.00000029 18.37499942 37.50000029 37.50000029 0 0 1-3.375 34.875 37.50000029 37.50000029 0 0 1-51.37499971 11.62500029c-4.12499971-2.62500029-7.12500029-6.37499971-11.25-8.62499971a67.87500029 67.87500029 0 0 0-37.50000029-12.375 37.50000029 37.50000029 0 0 0-24.00000029 12.375 26.62499971 26.62499971 0 0 0-7.12499942 23.99999942l13.87499942 52.12500029a200.99999971 200.99999971 0 0 1 94.5 27.74999971 269.62499971 269.62499971 0 0 1 72.37500029 64.87500058 225 225 0 0 1 37.50000029 74.99999971 249.37499971 249.37499971 0 0 1 9.37499942 86.625 275.24999971 275.24999971 0 0 1-14.99999942 72 281.99999971 281.99999971 0 0 1-112.5 137.99999971 299.99999971 299.99999971 0 0 1-124.12500029 43.50000058 310.5 310.5 0 0 1-91.125 0 299.99999971 299.99999971 0 0 1-164.25-87.00000029 322.49999971 322.49999971 0 0 1-85.12499971-297.37500029 316.50000029 316.50000029 0 0 1 189.74999971-225 50.24999971 50.24999971 0 0 1 42.00000029 2.25 37.50000029 37.50000029 0 0 1 15.37499971 35.25000029 37.50000029 37.50000029 0 0 1-24.00000029 30.75000029 241.49999971 241.49999971 0 0 0-153.37499942 193.5 245.99999971 245.99999971 0 0 0 13.5 119.25 242.25000029 242.25000029 0 0 0 112.5 129.74999942 219.375 219.375 0 0 0 102.375 24.75 255.75000029 255.75000029 0 0 0 85.87499942-15.37499971A203.625 203.625 0 0 0 714.87500029 662.00000029a166.5 166.5 0 0 0 14.24999971-33.75 187.49999971 187.49999971 0 0 0 0-104.25000058 158.24999971 158.24999971 0 0 0-47.62500029-73.125 208.50000029 208.50000029 0 0 0-31.5-22.87499942 120.375 120.375 0 0 0-31.5-12.00000058c7.49999971 29.99999971 15.75 60.00000029 23.625 90.37500029l3.75000029 21.75000029a127.87499971 127.87499971 0 0 1-103.12499971 129.74999942 122.625 122.625 0 0 1-90-16.875A135.37500029 135.37500029 0 0 1 399.5 572.00000029a163.87499971 163.87499971 0 0 1-9-49.12500058 169.49999971 169.49999971 0 0 1 28.50000029-103.5 187.49999971 187.49999971 0 0 1 104.24999971-72l-9-34.875a103.5 103.5 0 0 1 5.625-74.99999971 108.37500029 108.37500029 0 0 1 27-34.12500029A110.25 110.25 0 0 1 586.99999971 178.62499971a112.5 112.5 0 0 1 58.12500058-0.74999971z" fill="#e6e6e6" p-id="3178" /><path d="M490.24999971 451.62500029a87.75 87.75 0 0 0-22.12499971 41.99999942 112.5 112.5 0 0 0 0 43.50000058 64.87499971 64.87499971 0 0 0 24.75 40.5 47.62500029 47.62500029 0 0 0 37.50000029 7.12499942 52.49999971 52.49999971 0 0 0 42.37499971-48.74999942 135 135 0 0 0-3.375-17.25000029l-25.875-97.875a119.62500029 119.62500029 0 0 0-53.25000029 30.75000029z" fill="#ce3633" p-id="3179" /></svg>
            </nuxt-link>
          </button>
        </div>
        <button
          class="song-link hover:text-red-400"
        >
          <span v-if="currentSong">{{ currentSong.name }}</span>
        </button>
      </div>
      <div class="cd">
        <img class="image" :src="currentSong?.cover">
        <button
          class="toggle-button"
          @click.stop="playOrpluse"
        >
          <Icon v-if="ispaused" name="material-symbols:play-arrow-outline" />
          <Icon v-else name="ic:baseline-pause" />
        </button>
      </div>
      <div class="trigger">
        <Icon name="fluent:music-note-2-16-filled" />
      </div>
    </div>
  </client-only>
</template>

  <style lang="scss" scoped>
    #player {
      $size: 4rem;
      $panel-size: 11rem;
      position: fixed;
      left: 0;
      bottom: 18%;
      z-index: $z-index-toolbox;
      transition: opacity $transition-time-fast,
        transform $transition-time-normal cubic-bezier(0.65, 0.05, 0.36, 1);
      height: $size;
      transform: translateX(-15rem);
      display: flex;
      align-items: center;
      border-top-right-radius: $sm-radius;
      border-bottom-right-radius: $sm-radius;
      overflow: hidden;
      opacity: 0.5;
      &.playing {
        transform: translateX(-$panel-size);
      }
      &:hover {
        opacity: 0.9;
        transform: translateX(0);
      }

      .panel {
        width: $panel-size;
        padding: $gap;

        .control {
          display: flex;
          justify-content: flex-start;
          margin-bottom: $xs-gap;

          > .button {
            margin-right: $lg-gap;
            &.player {
              color: $music163-primary;
            }
          }
        }

        .song-link {
          max-width: 11rem;
          color: $text-secondary;
          @include color-transition();
          @include text-overflow();
        }
      }

      .cd {
        position: relative;
        display: block;
        width: $size;
        height: $size;
        overflow: hidden;

        .image {
          width: 100%;
          height: 100%;
          overflow: hidden;
          background-size: cover;
        }

        .toggle-button {
          position: absolute;
          top: 0;
          left: 0;
          width: 100%;
          height: 100%;
          border-radius: 100%;
          font-size: $font-size-h1;
          color: $white;
        }
      }

      .trigger {
        height: $size;
        display: flex;
        justify-content: center;
        align-items: center;
        writing-mode: tb-rl;
        color: $white;
        background-image: linear-gradient(0deg, #ff5a4c 0%, #ff1d12 100%);

        .iconfont {
          font-size: $font-size-h4;
        }
      }
    }
  </style>

